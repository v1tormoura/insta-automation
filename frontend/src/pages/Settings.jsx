        ...
        <div className="settings-grid">
          ...
          <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ duration:.25 }} style={cardStyle}>
            <div style={{ padding:'12px 16px', borderBottom:'1px solid var(--mf-border)' }}>
              <h3 style={{ fontSize:'.88rem', fontWeight:700, color:'var(--mf-text)', margin:0 }}>Automação</h3>
            </div>
            <div style={{ padding:'16px' }}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:16 }}>
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:13, fontWeight:700, color:'var(--mf-text)', marginBottom:4 }}>Headless Global</div>
                  <div style={{ fontSize:11, color:'var(--mf-text-3)', lineHeight:1.6 }}>Quando ativado, o navegador roda em segundo plano sem abrir janela visível.</div>
                </div>
                <div
                  onClick={() => updateHeadless(!settings.headless)}
                  style={{
                    width:44, height:24, borderRadius:99, cursor:'pointer', flexShrink:0,
                    background: settings.headless ? 'var(--mf-mod, var(--mf-accent-500))' : 'oklch(0.20 0.04 235 / 0.8)',
                ...
                >
                  <div style={{
                    ...
                    width:16, height:16, borderRadius:'50%', background:'var(--mf-text)',
          ...
          <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ duration:.25, delay:.06 }} style={cardStyle}>
            <div style={{ padding:'12px 16px', borderBottom:'1px solid var(--mf-border)' }}>
              <h3 style={{ fontSize:'.88rem', fontWeight:700, color:'var(--mf-text)', margin:0 }}>Modo atual</h3>
            </div>
            <div style={{ padding:'16px' }}>
              <div style={{ fontSize:20, fontWeight:800, marginBottom:8, color: settings.headless ? 'var(--mf-success-500)' : 'oklch(0.68 0.18 270)' }}>
                {settings.headless ? '🤫 Oculto / Headless' : '👁️ Visível / Navegador aberto'}
              </div>
              <p style={{ fontSize:12, color:'var(--mf-text-3)', lineHeight:1.6, margin:'0 0 16px' }}>
              ...
              <div style={{ borderTop:'1px solid var(--mf-border)', paddingTop:14 }}>
                <div style={{ fontSize:10, fontWeight:700, color:'var(--mf-text-3)', textTransform:'uppercase', letterSpacing:'.07em', marginBottom:10, fontFamily:'var(--mf-mono)' }}>Status do sistema</div>
                <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                  {SYS_ROWS.map((s, i) => (
                    <motion.div key={s.label} initial={{ opacity:0, x:-6 }} animate={{ opacity:1, x:0 }} transition={{ delay:.15 + i * .04 }}
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:8, fontSize:12, fontWeight:600, color:'var(--mf-text)' }}>
                        <span style={{ width:7, height:7, borderRadius:'50%', background: s.ok ? 'var(--mf-success-500)' : 'var(--mf-danger-500)', display:'inline-block', boxShadow: s.ok ? '0 0 6px var(--mf-success-500)' : '0 0 6px var(--mf-danger-500)' }} />
                        {s.label}
                      </div>
                      <span style={{ fontSize:10, fontWeight:700, padding:'1px 8px', borderRadius:99, background: s.ok ? 'oklch(0.22 0.06 150 / 0.6)' : 'oklch(0.22 0.06 15 / 0.6)', color: s.ok ? 'var(--mf-success-500)' : 'var(--mf-danger-500)', border:`1px solid ${s.ok ? 'oklch(0.38 0.12 150 / 0.35)' : 'oklch(0.38 0.12 15 / 0.35)'}` }}>
                        {s.ok ? 'OK' : 'Offline'}
                      </span>
                    </motion.div>
                  ))}
