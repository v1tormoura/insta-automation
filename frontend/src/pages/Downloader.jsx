...
export default function Downloader() {
  ...
  const pageActions = selected.size > 0 ? (
    <button className="btn-primary" style={{ display:'flex', alignItems:'center', gap:6, height:34, padding:'0 16px', borderRadius:8, fontSize:13 }}
      ...
      <motion.div initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} transition={{ duration:.22 }} style={{ ...cardStyle, padding:'16px 18px', marginBottom:14 }}>
        <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
          <input
            ...
            style={inputStyle}
          />
          <button className="btn-primary" style={{ height:38, padding:'0 20px', borderRadius:8, opacity:(loading || !username.trim()) ? .5 : 1 }}
        ...
        <>
          ...
          {hasMore && (
            <div style={{ textAlign:'center', marginBottom:20 }}>
              <button className="btn-ghost" style={{ padding:'9px 28px', fontSize:13, borderRadius:9 }} onClick={loadMore} disabled={loadingMore}>
