...
function FormField({ label, value, onChange, placeholder, type = 'text', hint, required }) {
  return (
    <div style={{ marginBottom: 14 }}>
      <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--mf-text-3)', letterSpacing: .5, textTransform: 'uppercase', display: 'block', marginBottom: 5 }}>
      ...
      <input
        ...
        style={{
          ...
          background: 'var(--bg3)', color: 'var(--mf-text)', fontSize: 13,
          fontFamily: 'var(--mf-mono)', outline: 'none',
        }}
      />
      {hint && <div style={{ fontSize: 11, color: 'var(--mf-text-3)', marginTop: 4 }}>{hint}</div>}
    </div>
  );
}
  ...
  return (
    <PageShell
      ...
      actions={
        <button
          onClick={openNew}
          style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 9, border: 'none', background: 'var(--mf-mod-publicar)', color: 'var(--mf-text)', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}
        >
    ...
    >
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .35 }}>
        <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14, padding: '20px 22px', marginBottom: 24 }}>
          <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--mf-mod-publicar)', letterSpacing: 1, marginBottom: 4, textTransform: 'uppercase' }}>Configuração do App Meta</div>
          <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--mf-text)', marginBottom: 4 }}>Copie esses valores no painel Meta Developers</div>
          <div style={{ fontSize: 12, color: 'var(--mf-text-3)', marginBottom: 16 }}>
            ...
            {configRows.map(({ label, value }) => (
              <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 9, padding: '10px 14px' }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--mf-text-3)', marginBottom: 2 }}>{label}</div>
                  <div style={{ fontSize: 12, fontFamily: 'var(--mf-mono)', color: 'var(--mf-text-2)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{value}</div>
                </div>
                <button
                  onClick={() => copy(value, label)}
                  style={{ flexShrink: 0, padding: '5px 10px', borderRadius: 7, border: '1px solid var(--border)', background: copiedKey === label ? 'color-mix(in oklch, var(--mf-success-500) 15%, transparent)' : 'var(--bg2)', color: copiedKey === label ? 'var(--mf-success-500)' : 'var(--mf-text-3)', fontSize: 11, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, transition: 'all .2s' }}
                >
      ...
      {/* ── Lista de apps ───────────────────────────────────── */}
      <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--mf-text-3)', letterSpacing: 1, marginBottom: 12, textTransform: 'uppercase' }}>
        Apps configurados
      </div>

      {loading && (
        <div style={{ padding: '40px 0', textAlign: 'center', color: 'var(--mf-text-3)', fontSize: 13 }}>Carregando...</div>
      )}
      ...
      {!loading && apps.length === 0 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center', padding: '48px 24px', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14 }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--mf-text)', marginBottom: 6 }}>Nenhum app configurado</div>
          <div style={{ fontSize: 13, color: 'var(--mf-text-3)', marginBottom: 20 }}>Adicione pelo menos um app Meta para usar o fluxo OAuth.</div>
          <button onClick={openNew} style={{ padding: '10px 22px', borderRadius: 9, border: 'none', background: 'var(--mf-mod-publicar)', color: 'var(--mf-text)', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>Adicionar primeiro app</button>
        </motion.div>
      )}
            ...
            <motion.div key={app._id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * .04 }}>
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}>
                  {/* icon */}
                  <div style={{ width: 40, height: 40, borderRadius: 10, background: app.isDefault ? 'color-mix(in oklch, var(--mf-mod-publicar) 15%, transparent)' : 'var(--bg3)', border: `1px solid ${app.isDefault ? 'color-mix(in oklch, var(--mf-mod-publicar) 30%, transparent)' : 'var(--border)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <IcoKey />
                  </div>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 4 }}>
                      <span style={{ fontSize: 15, fontWeight: 700, color: 'var(--mf-text)' }}>{app.name}</span>
                      {app.isDefault && (
                        <span style={{ fontSize: 10, fontWeight: 800, padding: '2px 8px', borderRadius: 20, background: 'color-mix(in oklch, var(--mf-mod-publicar) 15%, transparent)', color: 'var(--mf-mod-publicar)', letterSpacing: .5 }}>PADRÃO</span>
                      )}
                    </div>
                  ...
                  <div style={{ display: 'flex', gap: 8, flexShrink: 0, flexWrap: 'wrap' }}>
                    {!app.isDefault && (
                      <button
                        ...
                        style={{ padding: '6px 12px', borderRadius: 7, border: '1px solid color-mix(in oklch, var(--mf-mod-publicar) 35%, transparent)', background: 'color-mix(in oklch, var(--mf-mod-publicar) 8%, transparent)', color: 'var(--mf-mod-publicar)', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}
                      >
                        Usar como padrão
                      </button>
                    )}
                    <button
                      ...
                      style={{ width: 32, height: 32, borderRadius: 7, border: '1px solid var(--border)', background: 'var(--bg3)', color: 'var(--mf-text-2)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                      title="Editar"
                    >
                      <IcoEdit />
                    </button>
                    <button
                      ...
                      style={{ width: 32, height: 32, borderRadius: 7, border: '1px solid color-mix(in oklch, var(--mf-danger-500) 30%, transparent)', background: 'color-mix(in oklch, var(--mf-danger-500) 6%, transparent)', color: 'var(--mf-danger-500)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: deleting === app._id ? .5 : 1 }}
                      title="Remover"
                    >
                      <IcoTrash />
                    </button>
      ...
      {!loading && apps.length > 0 && (
        <div style={{ marginTop: 20, background: 'color-mix(in oklch, var(--mf-mod-publicar) 6%, transparent)', border: '1px solid color-mix(in oklch, var(--mf-mod-publicar) 15%, transparent)', borderRadius: 12, padding: '14px 16px', fontSize: 12, color: 'var(--mf-text-3)', lineHeight: 1.7 }}>
          ...
          <div className="modal" style={{ width: 'min(540px,100%)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <div>
                ...
                <div style={{ fontSize: 12, color: 'var(--mf-text-3)', marginTop: 3 }}>Credenciais do painel Meta Developers</div>
              </div>
              <button onClick={closeForm} style={{ background: 'none', border: 'none', color: 'var(--mf-text-2)', fontSize: 22, cursor: 'pointer', lineHeight: 1 }}>×</button>
            </div>
            ...
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--mf-text-3)', letterSpacing: .5, marginBottom: 14, textTransform: 'uppercase' }}>Campos opcionais</div>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 8 }}>
              <button onClick={closeForm} style={{ padding: '9px 20px', borderRadius: 9, border: '1px solid var(--border)', background: 'transparent', color: 'var(--mf-text-2)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Cancelar</button>
              <button
                onClick={saveForm}
                disabled={saving}
                style={{ padding: '9px 22px', borderRadius: 9, border: 'none', background: 'var(--mf-mod-publicar)', color: 'var(--mf-text)', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7 }}
              >
                {saving ? <><span style={{ width:14, height:14, border:'2px solid var(--mf-border-strong)', borderTopColor:'var(--mf-text)', borderRadius:'50%', display:'inline-block', animation:'spin .7s linear infinite' }} /> Salvando...</> : `${editId ? 'Salvar alterações' : 'Criar app'}`}
              </button>
            </div>
          </div>
        </div>
