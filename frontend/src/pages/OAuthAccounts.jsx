  ...
  if (!src || failed) {
    return (
      <div style={{
        ...
        fontSize: 18, fontWeight: 700, color: 'var(--mf-text-2)',
      }}>
        {initials}
      </div>
    );
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--mf-text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              @{account.username}
            </span>
            {account.accountType && (
          ...
          {account.name && account.name !== account.username && (
            <div style={{ fontSize: 11, color: 'var(--mf-text-3)', marginTop: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {account.name}
            </div>
          )}
          <div style={{ fontSize: 10, color: 'var(--mf-text-3)', marginTop: 3, fontFamily: 'var(--mf-mono)' }}>
      ...
      {(account.igUserId || account.tokenExpiresAt) && (
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {account.igUserId && (
            <div style={{ fontSize: 9, fontFamily: 'var(--mf-mono)', color: 'var(--mf-text-3)', background: 'var(--mf-border-subtle)', padding: '2px 6px', borderRadius: 4 }}>
              ID: {account.igUserId.slice(0, 12)}…
            </div>
          )}
          {account.tokenExpiresAt && (
            <div style={{ fontSize: 9, fontFamily: 'var(--mf-mono)', color: status === 'expired' ? 'var(--mf-danger-500)' : status === 'expiring' ? 'var(--mf-warning-500)' : 'var(--mf-text-3)', background: 'var(--mf-border-subtle)', padding: '2px 6px', borderRadius: 4 }}>
              {expiry}
            </div>
          )}
        </div>
      ...
      <div style={{ display: 'flex', gap: 6, marginTop: 2 }}>
        {status === 'none' || status === 'missing' ? (
          <button onClick={() => handleConnect(false)} disabled={busy}
            style={{ flex: 1, padding: '7px 10px', borderRadius: 8, border: '1px solid var(--mf-mod, var(--mf-accent-500))', background: 'color-mix(in oklch, var(--mf-mod-contas) 10%, transparent)', color: 'var(--mf-mod, var(--mf-accent-500))', cursor: busy ? 'wait' : 'pointer', fontSize: 12, fontWeight: 600, transition: '.15s' }}>
            {busy ? 'Aguarde…' : '🔗 Conectar'}
          </button>
        ) : status === 'expired' ? (
          <button onClick={() => handleConnect(true)} disabled={busy}
            style={{ flex: 1, padding: '7px 10px', borderRadius: 8, border: '1px solid color-mix(in oklch, var(--mf-danger-500) 40%, transparent)', background: 'color-mix(in oklch, var(--mf-danger-500) 10%, transparent)', color: 'var(--mf-danger-500)', cursor: busy ? 'wait' : 'pointer', fontSize: 12, fontWeight: 600, transition: '.15s' }}>
            {busy ? 'Aguarde…' : '🔄 Reconectar'}
          </button>
        ) : status === 'expiring' ? (
          <button onClick={() => handleConnect(true)} disabled={busy}
            style={{ flex: 1, padding: '7px 10px', borderRadius: 8, border: '1px solid color-mix(in oklch, var(--mf-warning-500) 30%, transparent)', background: 'color-mix(in oklch, var(--mf-warning-500) 8%, transparent)', color: 'var(--mf-warning-500)', cursor: busy ? 'wait' : 'pointer', fontSize: 12, fontWeight: 600, transition: '.15s' }}>
            {busy ? 'Aguarde…' : '🔄 Renovar'}
          </button>
        ) : (
          <button onClick={() => handleConnect(true)} disabled={busy}
            style={{ flex: 1, padding: '7px 10px', borderRadius: 8, border: '1px solid var(--border)', background: 'transparent', color: 'var(--mf-text-2)', cursor: busy ? 'wait' : 'pointer', fontSize: 12, fontWeight: 600, transition: '.15s' }}>
            {busy ? 'Aguarde…' : '🔄 Reconectar'}
          </button>
        )}
        ...
        {account.igUserId && (
          <button onClick={handleDisconnect} disabled={busy} title="Desconectar da API"
            style={{ padding: '7px 10px', borderRadius: 8, border: '1px solid var(--border)', background: 'transparent', color: 'var(--mf-text-3)', cursor: busy ? 'wait' : 'pointer', fontSize: 12, transition: '.15s' }}>
            ✕
          </button>
        )}
      </div>
...
function StatCard({ label, value, color = 'var(--mf-text)' }) {
  return (
    <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, padding: '14px 16px', flex: 1, minWidth: 100 }}>
      <div style={{ fontSize: 22, fontWeight: 800, color, fontFamily: 'var(--mf-mono)', letterSpacing: '-1px' }}>{value}</div>
      <div style={{ fontSize: 11, color: 'var(--mf-text-3)', marginTop: 2 }}>{label}</div>
    </div>
  );
}
...
export default function OAuthAccounts() {
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 24 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 36, height: 36, borderRadius: 10, background: 'color-mix(in oklch, var(--mf-mod-contas) 10%, transparent)', border: '1px solid color-mix(in oklch, var(--mf-mod-contas) 20%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>🔗</div>
            <div>
              <h1 style={{ fontSize: 20, fontWeight: 800, color: 'var(--mf-text)', margin: 0 }}>Conexões OAuth</h1>
              <p style={{ fontSize: 12, color: 'var(--mf-text-3)', margin: 0 }}>Gerencie tokens da Instagram Graph API por conta</p>
            </div>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          {metaApps.length > 1 && (
            <select value={selApp} onChange={e => setSelApp(e.target.value)}
              style={{ padding: '7px 10px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg2)', color: 'var(--mf-text-2)', fontSize: 12, cursor: 'pointer' }}>
              {metaApps.map(a => <option key={a._id} value={a._id}>{a.name || a.appId || a._id}</option>)}
            </select>
          )}
          <button onClick={load} style={{ padding: '7px 12px', borderRadius: 8, border: '1px solid var(--border)', background: 'transparent', color: 'var(--mf-text-2)', cursor: 'pointer', fontSize: 12 }}>
            ↻ Atualizar
          </button>
          <button onClick={connectNew}
            style={{ padding: '7px 16px', borderRadius: 8, border: 'none', background: 'var(--mf-mod, var(--mf-accent-500))', color: 'var(--mf-bg)', fontWeight: 700, cursor: 'pointer', fontSize: 12 }}>
            + Conectar nova conta
          </button>
        </div>
      </div>
      ...
      {/* Meta info box */}
      <div style={{ background: 'color-mix(in oklch, var(--mf-mod-contas) 4%, transparent)', border: '1px solid color-mix(in oklch, var(--mf-mod-contas) 12%, transparent)', borderRadius: 10, padding: '10px 14px', marginBottom: 20, fontSize: 11, color: 'var(--mf-text-3)', lineHeight: 1.6 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
        <input
          ...
          style={{ flex: 1, minWidth: 0, padding: '8px 12px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg2)', color: 'var(--mf-text)', fontSize: 12 }}
        />
      ...
      {/* Grid */}
      {loading ? (
        <div style={{ textAlign: 'center', padding: '60px 0', color: 'var(--mf-text-3)', fontSize: 13 }}>Carregando contas…</div>
      ) : filtered.length === 0 ? (
