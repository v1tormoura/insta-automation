      ...
      <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ duration:.25 }} style={{ ...cardStyle, marginBottom:14 }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 16px', borderBottom:'1px solid var(--mf-border)' }}>
          ...
          <button onClick={() => { setGoalInput(goal ? goal.toFixed(2) : ''); setEditGoal(true); }}
            style={{ fontSize: 'var(--mf-t-micro)', color:'var(--mf-mod, var(--mf-accent-500))', background:'none', border:'none', cursor:'pointer', fontWeight:700 }}>
        ...
        <div style={{ padding:'16px 18px' }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom:16 }}>
            <div>
              <div style={{ fontSize: 'var(--mf-t-display)', fontWeight:900, color:'var(--mf-mod, var(--mf-accent-500))', letterSpacing:'-1px', fontVariantNumeric:'tabular-nums' }}>{fmtBRL(total)}</div>
              <div style={{ fontSize: 'var(--mf-t-xs)', color:'var(--mf-text-3)', marginTop:2 }}>
            ...
            <div style={{ textAlign:'right' }}>
              <div style={{ fontSize: 'var(--mf-t-display)', fontWeight:900, color: reached ? 'var(--mf-success-500)' : 'var(--mf-text)', letterSpacing:'-1px', fontVariantNumeric:'tabular-nums' }}>{pct.toFixed(0)}%</div>
              <div style={{ fontSize: 'var(--mf-t-xs)', color:'var(--mf-text-3)', marginTop:2, fontFamily:'var(--mf-mono)' }}>{thisMonth.length} {thisMonth.length === 1 ? 'venda' : 'vendas'}</div>
            </div>
          </div>
          ...
          <div style={{ fontSize: 'var(--mf-t-micro)', color: reached ? 'var(--mf-success-500)' : 'var(--mf-text-3)' }}>
            {reached
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(280px, 1fr))', gap:14 }}>
        ...
        <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ duration:.25, delay:.06 }} style={cardStyle}>
          ...
          <div style={{ padding:'14px 16px' }}>
            <form onSubmit={addEntry} style={{ display:'flex', flexDirection:'column', gap:12 }}>
              ...
              <div>
                <label style={{ fontSize: 'var(--mf-t-micro)', color:'var(--mf-text-3)', display:'block', marginBottom:5, fontFamily:'var(--mf-mono)', textTransform:'uppercase', letterSpacing:'.05em' }}>Descrição</label>
                <input
        ...
        <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ duration:.25, delay:.1 }} style={cardStyle}>
          ...
          <div style={{ maxHeight:300, overflowY:'auto' }}>
            {thisMonth.length === 0 ? (
              <div style={{ textAlign:'center', padding:'32px 16px', color:'var(--mf-text-3)', fontSize: 'var(--mf-t-xs)' }}>
              ...
              [...thisMonth].reverse().map((e, i) => (
                <div key={e.id} style={{
                  ...
                  <div>
                    <div style={{ fontSize: 'var(--mf-t-xs)', fontWeight:600, color:'var(--mf-text)' }}>{e.desc}</div>
                    <div style={{ fontSize: 'var(--mf-t-nano)', color:'var(--mf-text-3)', marginTop:1, fontFamily:'var(--mf-mono)' }}>
