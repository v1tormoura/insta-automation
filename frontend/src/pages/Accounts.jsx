          ...
          <div className="modal" style={{ width: 'min(520px,100%)' }}>
            ...
            {!oauthWaiting ? (
              <>
                ...
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                    <div style={{ width: 24, height: 24, borderRadius: '50%', background: 'rgba(0,212,255,.1)', border: '1px solid rgba(0,212,255,.2)', color: 'var(--text3)', fontSize: 12, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>2</div>
                    <span style={{ fontWeight: 700, fontSize: 14, color: 'var(--text2)' }}>Autorize no navegador isolado</span>
                  </div>
                  <div style={{ fontSize: 12, color: 'var(--text3)', lineHeight: 1.6 }}>
                    Após clicar em <strong style={{ color: 'var(--text2)' }}>Copiar</strong>, cole o link no Dolphin/AdsPower, faça login e autorize o app. A conta conecta automaticamente.
                  </div>
                </div>
              ...
              /* Aguardando autorização via SSE */
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18, padding: '20px 0 8px' }}>
                <div style={{ width: 52, height: 52, borderRadius: '50%', border: '3px solid rgba(0,212,255,.15)', borderTopColor: 'var(--cyan)', animation: 'mf-spin 0.9s linear infinite' }} />
                <div style={{ textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text2)', marginTop: 6, lineHeight: 1.6 }}>
                    Abra o link no Dolphin/AdsPower, faça login e autorize.<br />
                ...
                <div style={{ width: '100%', borderTop: '1px solid var(--border)', paddingTop: 16 }}>
                  <div style={{ fontSize: 11, color: 'var(--text2)', marginBottom: 8 }}>Ou cole a URL de retorno manualmente:</div>
                  <textarea
                    value={callbackUrl}
                    onChange={e => { setCallbackUrl(e.target.value); setOauthError(''); }}
                    placeholder="https://instaflow.pro:3001/api/oauth/callback?code=..."
                    rows={2}
                    style={{ width: '100%', boxSizing: 'border-box', padding: '8px 10px', borderRadius: 8, border: `1px solid ${oauthError ? 'rgba(239,68,68,.5)' : 'var(--border)'}`, background: 'var(--bg3)', color: 'var(--text)', fontSize: 11, fontFamily: 'monospace', resize: 'none', outline: 'none' }}
                  />
                  {oauthError && <div style={{ fontSize: 11, color: '#f87171', marginTop: 4 }}>⚠️ {oauthError}</div>}
                  {callbackUrl.trim() && (
                    <button onClick={handleManualConnect} disabled={oauthConnecting}
                ...
                <button onClick={() => setOauthWaiting(false)}
