...
const modalBox     = { background:'oklch(0.14 0.04 235 / 0.98)', border:'1px solid var(--mf-border)', borderRadius:16, padding:'20px 24px', width:'min(460px,calc(100vw - 32px))', boxSizing:'border-box', boxShadow:'0 24px 60px oklch(0 0 0 / 0.6)' };
const inp = { width:'100%', height:40, padding:'0 12px', borderRadius:8, border:'1px solid var(--mf-border)', background:'oklch(0.10 0.03 235 / 0.8)', color:'var(--mf-text)', fontSize:13, boxSizing:'border-box', outline:'none' };
    ...
    <>
      ...
      <PageShell icon={pageIcon} title="Biblioteca de Mídias" subtitle="Organize seus vídeos e imagens em pastas para reutilizar nas postagens." accent="purple" actions={pageActions}>
        ...
        <div className="layout-media-lib">
          ...
          {/* Sidebar de pastas */}
          <div style={{ background:'oklch(0.12 0.04 235 / 0.8)', border:'1px solid var(--mf-border)', borderRadius:14, overflow:'hidden', position:'sticky', top:20 }}>
            <div style={{ padding:'12px 14px', borderBottom:'1px solid var(--mf-border)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              <span style={{ fontSize:10, fontWeight:700, color:'var(--mf-text-3)', textTransform:'uppercase', letterSpacing:'.06em', fontFamily:'var(--mf-mono)' }}>Pastas</span>
            ...
            <div style={{ padding:'8px' }}>
              {folders.length === 0 && (
                <div style={{ padding:'12px 10px', fontSize:12, color:'var(--mf-text-3)', textAlign:'center' }}>Clique em "+" para criar uma pasta.</div>
              )}
              {folders.map(f => (
                ...
                >
                  <div style={{ display:'flex', alignItems:'center', gap:7, minWidth:0 }}>
                    ...
                    <span style={{ fontSize:12, fontWeight: activeFolder === f ? 600 : 400, color: activeFolder === f ? 'oklch(0.80 0.16 270)' : 'var(--mf-text-2)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{f}</span>
                  </div>
                  <div style={{ display:'flex', alignItems:'center', gap:4, flexShrink:0 }}>
                    <span style={{ fontSize:10, color:'var(--mf-text-3)', background:'oklch(0.10 0.03 235 / 0.5)', borderRadius:10, padding:'1px 6px' }}>{folderCounts[f] || 0}</span>
                    <button onClick={e => { e.stopPropagation(); deleteFolder(f); }} style={{ width:17, height:17, borderRadius:4, background:'transparent', border:'none', color:'var(--mf-text-3)', cursor:'pointer', fontSize:10, display:'flex', alignItems:'center', justifyContent:'center' }}>✕</button>
                  </div>
                </div>
              ))}
            </div>
            <div style={{ borderTop:'1px solid var(--mf-border)', padding:'10px 14px', display:'flex', flexDirection:'column', gap:5 }}>
              {[
                ...
                <div key={s.l} style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                  <span style={{ fontSize:11, color:'var(--mf-text-3)' }}>{s.l}</span>
          ...
          <div>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
              <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                ...
                <div>
                  <div style={{ fontSize:14, fontWeight:700, color:'var(--mf-text)' }}>{activeFolder || 'Todos os arquivos'}</div>
                  <div style={{ fontSize:11, color:'var(--mf-text-3)' }}>{shown.length} arquivo(s)</div>
                </div>
              </div>
            </div>
            ...
            {/* Drop zone */}
            <label
              style={{ marginBottom:14, cursor:'pointer', display:'flex', flexDirection:'column', alignItems:'center', padding:'20px', borderRadius:12, border:`2px dashed ${dragOver ? 'var(--mf-mod, var(--mf-accent-500))' : 'var(--mf-border-strong)'}`, background: dragOver ? 'oklch(0.72 0.19 196 / 0.06)' : 'oklch(0.12 0.04 235 / 0.4)', transition:'.15s' }}
            ...
            >
              ...
              <strong style={{ fontSize:13, color:'var(--mf-text)' }}>{uploading ? 'Enviando...' : activeFolder ? `Arraste para "${activeFolder}" ou clique` : 'Arraste arquivos ou clique'}</strong>
              <span style={{ fontSize:12, color:'var(--mf-text-3)', marginTop:4 }}>MP4, MOV, JPG, PNG · Múltiplos arquivos</span>
            </label>
            ...
            {shown.length > 0 ? (
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(148px,1fr))', gap:10 }}>
                {shown.map((item, i) => (
                  <motion.div key={item._id} initial={{ opacity:0, y:6 }} animate={{ opacity:1, y:0 }} transition={{ delay:i*.02 }}
                    style={{ background:'oklch(0.14 0.04 235 / 0.8)', border:'1px solid var(--mf-border)', borderRadius:12, overflow:'hidden' }}
                  ...
                  >
                    <div onClick={() => setPreview(item)} style={{ width:'100%', aspectRatio:'1', background:'oklch(0.10 0.03 235)', cursor:'pointer', position:'relative', overflow:'hidden' }}>
                      ...
                      <span style={{ position:'absolute', top:6, left:6, fontSize:10, fontWeight:700, padding:'2px 6px', borderRadius:5, background: item.type === 'video' ? 'oklch(0.68 0.18 270 / 0.85)' : 'oklch(0.72 0.18 150 / 0.85)', color:'var(--mf-text)', letterSpacing:.4 }}>
                        {item.type === 'video' ? 'VID' : 'IMG'}
                      </span>
                    </div>
                    <div style={{ padding:'8px 10px' }}>
                      <div style={{ fontSize:11, fontWeight:600, color:'var(--mf-text)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }} title={item.originalName}>{item.originalName}</div>
                      <div style={{ fontSize:10, color:'var(--mf-text-3)', marginTop:2 }}>{fmt(item.size)}</div>
                    </div>
                    <div style={{ display:'flex', gap:6, padding:'0 10px 10px' }}>
                      <button onClick={() => { setMoveItem(item); setMoveTarget(folders.find(f => f !== item.folder) || folders[0] || ''); }}
                        style={{ flex:1, fontSize:11, padding:'5px 0', borderRadius:7, border:'1px solid var(--mf-border)', background:'oklch(0.10 0.03 235 / 0.5)', color:'var(--mf-text-2)', cursor:'pointer' }}>
                        Mover
                      </button>
                      <button onClick={() => deleteFile(item._id)}
                        style={{ flex:1, fontSize:11, padding:'5px 0', borderRadius:7, border:'1px solid oklch(0.38 0.12 15 / 0.3)', background:'oklch(0.22 0.06 15 / 0.2)', color:'var(--mf-danger-500)', cursor:'pointer' }}>
                        Excluir
                      </button>
                    </div>
                  </motion.div>
              ...
              <div style={{ textAlign:'center', padding:'48px 20px', color:'var(--mf-text-3)', background:'oklch(0.12 0.04 235 / 0.4)', borderRadius:14, border:'1px dashed var(--mf-border)' }}>
                <div style={{ fontSize:40, marginBottom:10 }}>📂</div>
                <div style={{ fontSize:14, fontWeight:600, color:'var(--mf-text-2)' }}>Pasta vazia</div>
          ...
          <div style={modalOverlay} onClick={e => e.target === e.currentTarget && setNewFolderOpen(false)}>
            <div style={modalBox}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
                ...
                <button onClick={() => setNewFolderOpen(false)} style={{ background:'none', border:'none', color:'var(--mf-text-3)', fontSize:22, cursor:'pointer' }}>×</button>
              </div>
              <input style={inp} placeholder="Ex.: clientes, reels-jan, stories"
                value={newFolderName} onChange={e => setNewFolderName(e.target.value)}
                onKeyDown={e => e.key === 'Enter' && createFolder()} autoFocus />
              <div style={{ fontSize:11, color:'var(--mf-text-3)', marginTop:6 }}>Use letras minúsculas, números e hífens.</div>
          ...
          <div style={modalOverlay} onClick={e => e.target === e.currentTarget && setMoveItem(null)}>
            <div style={modalBox}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
                ...
                <button onClick={() => setMoveItem(null)} style={{ background:'none', border:'none', color:'var(--mf-text-3)', fontSize:22, cursor:'pointer' }}>×</button>
              </div>
              <p style={{ fontSize:13, color:'var(--mf-text-2)', margin:'0 0 12px' }}>
        ...
        {preview && (
          <div style={{ ...modalOverlay, cursor:'zoom-out' }} onClick={() => setPreview(null)}>
            <div onClick={e => e.stopPropagation()} style={{ maxWidth:'90vw', maxHeight:'90vh', borderRadius:12, overflow:'hidden', background:'#000', position:'relative' }}>
              <button onClick={() => setPreview(null)} style={{ position:'absolute', top:10, right:10, background:'oklch(0 0 0 / 0.6)', border:'none', color:'var(--mf-text)', borderRadius:'50%', width:32, height:32, cursor:'pointer', fontSize:16, zIndex:10 }}>×</button>
              {preview.type === 'video' ? (
        ...
        {confirmModal && (
          <div style={{ ...modalOverlay, zIndex:10000 }} onClick={e => e.target === e.currentTarget && setConfirmModal(null)}>
            <div style={{ background:'linear-gradient(160deg, oklch(0.12 0.04 235), oklch(0.09 0.03 235))', border:'1px solid oklch(0.38 0.12 15 / 0.25)', borderRadius:18, padding:'28px 28px 24px', width:'min(420px,92vw)', boxShadow:'0 24px 60px oklch(0 0 0 / 0.6)' }}>
              ...
              <h3 style={{ margin:'0 0 8px', fontSize:17, fontWeight:700, color:'var(--mf-text)' }}>
                {confirmModal.type === 'folder' ? 'Excluir pasta' : 'Excluir mídia'}
              </h3>
              <p style={{ fontSize:13, color:'var(--mf-text-2)', margin:'0 0 4px', lineHeight:1.55 }}>
                {confirmModal.type === 'folder' ? 'Tem certeza que deseja excluir a pasta:' : 'Tem certeza que deseja excluir:'}
              </p>
              <div style={{ fontSize:13, fontWeight:600, color:'var(--mf-text)', background:'oklch(0.22 0.06 15 / 0.15)', border:'1px solid oklch(0.38 0.12 15 / 0.2)', borderRadius:8, padding:'8px 12px', margin:'6px 0 8px', wordBreak:'break-all' }}>
                {confirmModal.type === 'folder' ? `📁 ${confirmModal.name}` : confirmModal.name}
              </div>
              {confirmModal.type === 'folder'
                ? <p style={{ fontSize:12, color:'var(--mf-text-3)', margin:0 }}>As mídias desta pasta não serão excluídas.</p>
                : <p style={{ fontSize:12, color:'var(--mf-text-3)', margin:0 }}>Esta ação não pode ser desfeita.</p>
              }
              ...
              <div style={{ display:'flex', gap:10, justifyContent:'flex-end' }}>
                ...
                <button
                  ...
                  style={{ padding:'9px 22px', borderRadius:10, fontSize:13, fontWeight:700, cursor:'pointer', background:'linear-gradient(135deg,var(--mf-danger-500),var(--mf-danger-500))', border:'1px solid oklch(0.38 0.12 15 / 0.4)', color:'var(--mf-text)', boxShadow:'0 4px 14px oklch(0.38 0.12 15 / 0.35)' }}
                >
                  Excluir
                </button>
              </div>
