...
function traduzirErro(msg) {
  ...
  if (/checkpoint|challenge.*required/i.test(m))
    return 'Verificação pendente — abra o app do Instagram';
  ...
  if (/feedback_required|action.*blocked/i.test(m))
  ...
  if (/performing too many actions|too many actions/i.test(m))
    return 'Limite de publicação da API atingido — aguarde a janela de 24h liberar';
